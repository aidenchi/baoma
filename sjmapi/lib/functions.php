...
function getGoodsArray($item){
	...
	$goods['share_content']=emptyTag($item['name']).get_domain().str_replace("sjmapi/","",url("tuan","deal",array("id"=>$item['id'])));
...
function m_youhuiItem($item){
	...
	return array("id"=>$item['id'],
                                    ...
                                    "image_3_h"=>intval($item['image_3_h']),
											...
											"ypoint"=>$item['ypoint'],
											"address"=>$item['api_address'],
											"content"=>get_abs_url_root($item['content']),
									"is_sc"=>$is_sc,
...
function m_merchantItem($item){
	...
	return array("id"=>$item['id'],
									...
									"distance"=>round($item['distance']),
									"brief"	=>	strip_tags(get_abs_url_root($item['brief'])),
...
function get_collect_list($limit,$user_id)
	...
	function biz_check($email,$pwd)
	{
		if($email&&$pwd)
		{
			...
			$user_info = $GLOBALS['db']->getRow("select s.name as name,a.account_name as account_name,a.login_ip as login_ip ,a.login_time as login_time ,a.update_time as create_time, a.supplier_id as supplier_id,a.id,a.account_password from ".DB_PREFIX."supplier_account as a left join ".DB_PREFIX."supplier as s on a.supplier_id = s.id where a.account_name ='".$email."'");
			...
			if($is_use_pass)
			{
				$account_locations = $GLOBALS['db']->getAll("select location_id from ".DB_PREFIX."supplier_account_location_link where account_id = ".$user_info['id']);
	...
	function mobile_reg($mobile,$pwd,$ref_uid){
		$root=array();
		$have_user_id = $GLOBALS['db']->getOne("select id from ".DB_PREFIX."user where mobile = '$mobile'");
		if($have_user_id){
			...
			//自动获取会员分组
			$data['group_id'] = $GLOBALS['db']->getOne("select id from ".DB_PREFIX."user_group order by score asc limit 1");
			$data['is_effect'] = 1; //手机注册自动生效
			//print_r($data); exit;
			$GLOBALS['db']->autoExecute(DB_PREFIX."user",$data,"INSERT","");
			//print_r($GLOBALS['db']->error());
			...
			if($user_id > 0)
			{
				...
				$root['mobile'] = $data['mobile'];
				$root['user_pwd'] = $data['user_pwd'];
				$root['user_email'] = '';
				$root['user_money'] = 0;
