...
class mobile_qrcode{
	...
	public function decode_goods($pc_url)
	{
		...
		if (strpos($pc_url, 'ctl=goods') || strpos($pc_url, '/goods/')){
			$id = $this->get_id($pc_url,'id');
			if ($id > 0){
				$type = 15;
			}else{
				$type = 0;
	...
	public function decode_yh($pc_url)
	{
		...
		if (strpos($pc_url, '/youhui') == false){
			...
			//
			if (strpos($pc_url, '/store/') || strpos($pc_url, 'ctl=store')){				
				//&id=19 || id-19
			...
			}else if (strpos($pc_url, '/edetail/')||strpos($pc_url, 'ctl=edetail')){
				$id = $this->get_id($pc_url,'id');
				if ($id > 0){
					$type = 16;
				}else{
				...
				if (strpos($pc_url, 'fdetail') == false){
					$id = $this->get_id($pc_url,'cid');
					$type = 12;
