...
class my_order_list{
	public function index()
	{
		...
		if($user_id>0)
		{
			...
			if ($totalRows == 0){		
				$totalRows = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal_order where user_id = ".$user_id." and type = 0 and is_delete = 0 {$pay_status_and}");	
			...
			$list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal_order where user_id = ".$user_id." and type = 0 and is_delete = 0 {$pay_status_and} order by create_time desc limit ".$limit);		
