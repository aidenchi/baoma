...
{		
		//开始数据验证
		$res = array('status'=>1,'info'=>'','data'=>'','user_pwd'=>''); //用于返回的数据
		...
		if($res['status']>0)
		{
			...
			if(strim($GLOBALS['request']['qq_id'])!='')
			{
				...
				{
					$GLOBALS['db']->query("update ".DB_PREFIX."user set qq_id = '".strim($GLOBALS['request']['qq_id'])."' where id = ".$user_id);				
				}
			}
