...
function Qq()
{
	es_session::start();		
		...
		$qq_id = trim($GLOBALS['request']['qq_id']);
		$access_token = trim($GLOBALS['request']['access_token']);
		

		if($GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where qq_id = '".$qq_id."'")==0)
    		$GLOBALS['db']->query("update ".DB_PREFIX."user qq_id = '".$qq_id."' where id =".intval($GLOBALS['user_info']['id']));				
		elseif(intval($GLOBALS['db']->getOne("select id from ".DB_PREFIX."user where qq_id = '".$qq_id."'"))==intval($GLOBALS['user_info']['id']))
   			{
   				$GLOBALS['db']->query("update ".DB_PREFIX."user set qq_id = '".$qq_id."' where id =".intval($GLOBALS['user_info']['id']));							
   			}
   		else
   			{
   				$root['return'] = 0;	
				$root['info'] = "该QQ账号已被其他会员绑定";	
				output($root);
   			}
   		$root['return'] = 1;	
		$root['info'] = "绑定成功";	
