	...
	public function index()
	{
		...
		*/
		...
		if($is_login == 0 && $have_user_id){
			...
			$smsSubscribe = $GLOBALS['db']->getRow("select `id`,`mobile_phone`,`code`,`status`,`send_count`,`add_time` from ".DB_PREFIX."sms_mobile_verify where mobile_phone = '$mobile_phone' and type=0 order by id desc");
			
			$new_time=get_gmtime();
			$difftime=$new_time-$smsSubscribe['add_time'];
			if($smsSubscribe && intval($smsSubscribe['send_count']) <= 1 && intval($smsSubscribe['status'])== 0 && $difftime <61)
			{
