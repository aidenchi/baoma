...
class Paypal_payment implements payment {
	...
	public function get_payment_code($payment_notice_id)
	{
		...
		$data_return_url = get_domain().APP_ROOT.'/payment.php?act=response&class_name=Paypal';
	...
	public function response($request)
	{
		...
		curl_setopt($ppCurl, CURLOPT_URL, "http://www.paypal.com/cgi-bin/webscr"); // set url
		...
		if (strcmp ($lines[0],"SUCCESS") == 0)
		{
			...
			foreach($lines as $kk=>$vv){
				$val = explode("=",$vv);
				$request[trim($val[0])] = trim(urldecode($val[1]));
			}
