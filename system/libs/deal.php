...
function check_deal_number($id,$number = 0,$type='')
{
	...
	//4. 本团购当前会员未付款的数量
	$deal_user_unpaid_count = intval($GLOBALS['db']->getOne("select sum(oi.number) from ".DB_PREFIX."deal_order_item as oi left join ".DB_PREFIX."deal_order as o on oi.order_id = o.id where o.user_id = ".intval($GLOBALS['user_info']['id'])." and o.pay_status <> 2 and o.refund_status<>2 and oi.deal_id = ".$id." and o.is_delete = 0"));
	//5. 本团购已付款的数量
	//$deal_paid_count = intval($GLOBALS['db']->getOne("select sum(oi.number) from ".DB_PREFIX."deal_order_item as oi left join ".DB_PREFIX."deal_order as o on oi.order_id = o.id where  o.pay_status = 2 and oi.deal_id = ".$id." and o.is_delete = 0"));
	//5. 本团购非当前会员已下单未付款的数量
	$deal_user_nopaid_count = intval($GLOBALS['db']->getOne("select sum(oi.number) from ".DB_PREFIX."deal_order_item as oi left join ".DB_PREFIX."deal_order as o on oi.order_id = o.id where o.user_id = ".intval($GLOBALS['user_info']['id'])." and o.pay_status = 0 and oi.deal_id = ".$id." and o.is_delete = 0"));
	//6. 本团购部分付款的数量
	$deal_partpaid_count = intval($GLOBALS['db']->getOne("select sum(oi.number) from ".DB_PREFIX."deal_order_item as oi left join ".DB_PREFIX."deal_order as o on oi.order_id = o.id where   o.pay_status = 1 and oi.deal_id = ".$id." and o.refund_status<>2 and o.is_delete = 0"));
	...
	if($type === 'done')
	{
		if($deal_user_cart_count + $deal_buy_count + $deal_partpaid_count + $number > $deal_info['max_bought'] && $deal_info['max_bought'] > 0)
		{		
			$result['status'] = 0;
			$result['data'] = DEAL_OUT_OF_STOCK;  //库存不足
			$result['info'] = $deal_info['sub_name']." ".sprintf($GLOBALS['lang']['DEAL_MAX_BOUGHT'],$deal_info['max_bought']);
	...
	{
		if($deal_user_nopaid_count + $deal_buy_count + $deal_partpaid_count + $number > $deal_info['max_bought'] && $deal_info['max_bought'] > 0)
		{	
			$result['status'] = 0;
			$result['data'] = DEAL_OUT_OF_STOCK;  //库存不足
			$result['info'] = $deal_info['sub_name']." ".sprintf($GLOBALS['lang']['DEAL_MAX_BOUGHT'],$deal_info['max_bought']);
		...
		if($deal_user_nopaid_count + $deal_user_paid_count + $number < $deal_info['user_min_bought'] && $deal_info['user_min_bought'] > 0&&ACTION_NAME!='addcart')
		{
			$result['status'] = 0;
			$result['data'] = DEAL_ERROR_MIN_USER_BUY;  //用户最小购买数不足
