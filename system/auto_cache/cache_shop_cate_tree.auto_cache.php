	...
	public function load($param)
	{
		...
		if($cate_list===false)
		{
			...
			foreach($cate_list as $k=>$v)
			{
				...
				$ids = $ids_util->getChildIds($v['id'],"id","pid",$sort);
				$ids[] = $v['pid'];
				...
				else
				{
					...
					$sub_ids = $ids_util->getChildIds($v['id'],"id","pid",$sort);
					$sub_ids[] = $v['id'];
					
					$cate_list[$k]['count'] = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal where is_delete = 0 and is_effect = 1 and buy_type =0 and shop_cate_id in (".implode(",",$sub_ids).")");
				}
			}	
			$tree_util = new tree();
