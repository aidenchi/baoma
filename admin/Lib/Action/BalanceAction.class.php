		...
		{			
			...
			{				
				...
				foreach($dataList as $k=>$v)
				{
					...
					$pageBalance+=$v['add_balance_price'];
				}				
				
				$tmp_map = $map;
				$tmp_map['is_balance'] = 0;
				$totalBalance0 = M("DealCoupon")->where($tmp_map)->sum("balance_price");
				$totalBalance00 = M("DealCoupon")->where($tmp_map)->sum("add_balance_price");
				$tmp_map['is_balance'] = 1;
				$totalBalance1 = M("DealCoupon")->where($tmp_map)->sum("balance_price");
				$totalBalance11 = M("DealCoupon")->where($tmp_map)->sum("add_balance_price");
				$tmp_map['is_balance'] = 2;
				$totalBalance2 = M("DealCoupon")->where($tmp_map)->sum("balance_price");
				$totalBalance22 = M("DealCoupon")->where($tmp_map)->sum("add_balance_price");
				
				$this->assign("totalBalance0",$totalBalance0+$totalBalance00+$totalBalance1+$totalBalance11);
				$this->assign("totalBalance1",$totalBalance1+$totalBalance11);
				$this->assign("totalBalance2",$totalBalance2+$totalBalance22);
			...
			else
			{
				...
				$totalBalance00 = M("DealOrderItem")->where($tmp_map)->sum("add_balance_price_total");
				$tmp_map['is_balance'] = 1;
				$totalBalance1 = M("DealOrderItem")->where($tmp_map)->sum("balance_total_price");
				$totalBalance11 = M("DealOrderItem")->where($tmp_map)->sum("add_balance_price_total");
				$tmp_map['is_balance'] = 2;
				$totalBalance2 = M("DealOrderItem")->where($tmp_map)->sum("balance_total_price");
				$totalBalance22 = M("DealOrderItem")->where($tmp_map)->sum("add_balance_price_total");
				...
				foreach($dataList as $k=>$v)
				{
					...
					$pageBalance+=$v['add_balance_price_total'];
				}	
				$this->assign("totalBalance3",$pageBalance);
