...
class DealOrderAction extends CommonAction{
	public function incharge_index()
	{
		...
		{		
			$ids = M("User")->where(array("user_name"=>array('eq',trim($_REQUEST['user_name']))))->field("id")->findAll();
			$ids_arr = array();
			foreach($ids as $k=>$v)
			{
...
public function deal_index()
	{
		...
		if(trim($_REQUEST['user_name'])!='')
		$where.=" and ".DB_PREFIX."deal_order.user_name = '".trim($_REQUEST['user_name'])."'";
		...
		{
			$where.= " and ".DB_PREFIX."deal_order.order_sn = '".trim($_REQUEST['order_sn'])."' ";
		}
	...
	public function do_delivery()
	{
		...
		else
		{
			...
			{
				$deal_info =$GLOBALS['db']->getRow("select d.* from ".DB_PREFIX."deal as d left join ".DB_PREFIX."deal_order_item as doi on doi.deal_id = d.id where doi.id = ".$order_deal_id);
	...
	public function over_order()
	{
		...
		$order_info = M("DealOrder")->where("id=".$order_id." and is_delete = 0 and type = 0 and order_status = 0 and (pay_status = 2 and ((delivery_status = 2 or delivery_status = 0 or delivery_status = 5)) or (pay_amount = refund_money) or after_sale=1)")->find();
		
		if(!$order_info)
		{
			$this->error(l("INVALID_ORDER"));
		}
		M("DealOrder")->where("id=".$order_id." and is_delete = 0 and type = 0 and order_status = 0 and (pay_status = 2 and ((delivery_status = 2 or delivery_status = 0  or delivery_status = 5)) or (pay_amount = refund_money) or after_sale=1)")->setField("order_status",1);
		M("DealOrder")->where("id=".$order_id)->setField("update_time",get_gmtime());
	...
	{
		$order_id  = intval($_REQUEST['id']);
		$order_info = M("DealOrder")->where("id=".$order_id." and is_delete = 0 and type = 0 and order_status = 1 and (pay_status = 2 and ((delivery_status = 2 or delivery_status = 0 or delivery_status = 5)) or (pay_amount = refund_money))")->find();
		if(!$order_info)
		{
			$this->error(l("INVALID_ORDER"));
		}
		M("DealOrder")->where("id=".$order_id." and is_delete = 0 and type = 0 and order_status = 1 and (pay_status = 2 and ((delivery_status = 2 or delivery_status = 0 or delivery_status = 5)) or (pay_amount = refund_money))")->setField("order_status",0);
		M("DealOrder")->where("id=".$order_id)->setField("update_time",get_gmtime());
	...
	public function admin_memo()
	{
		...
		$after_sale = intval($_REQUEST['after_sale']);
		...
		if($after_sale>0)
		{
			...
			//处理商品数量
			//更新订单详情表，用于结算
			$sql = "update ".DB_PREFIX."deal_order_item set refund_status=1 where order_id= ".$order_id;
			$GLOBALS['db']->query($sql);
			if(intval($order_info['pay_status']) == 2)
			{
				$order_info_detail = $GLOBALS['db']->getAll("select deal_id,number from ".DB_PREFIX."deal_order_item where order_id=".$order_id);
				$stock_status=true;//用于数据回滚
				foreach($order_info_detail as $kk =>$vv)
				{
					$sql = "update ".DB_PREFIX."deal set buy_count = buy_count - ".intval($vv['number'])." where id= ".intval($vv['deal_id']);
					$GLOBALS['db']->query($sql);
					...
					{
						$affect_list[] = $vv;
						$affect_deal = $GLOBALS['db']->getRow("select buy_count from ".DB_PREFIX."deal where id= ".intval($vv['deal_id']));
						if($affect_deal['buy_count'] == 0)
						{
							$sql_1 = "update ".DB_PREFIX."deal set buy_status = 0 where id= ".intval($vv['deal_id']);
						}
						else
						{
							$sql_1 = "update ".DB_PREFIX."deal set buy_status = 1 where id= ".intval($vv['deal_id']);
						}
				...
				if(!$stock_status)
				{
					...
					{
						$sql = "update ".DB_PREFIX."deal set buy_count = buy_count + ".intval($v['number'])." where id= ".intval($v['deal_id']);
						
						$affect_deal = $GLOBALS['db']->getRow("select buy_count,max_bought from ".DB_PREFIX."deal where id= ".intval($v['deal_id']));
						if($affect_deal['buy_count'] == $affect_deal['max_bought'])
						{
							$sql_1 = "update ".DB_PREFIX."deal set buy_status = 2 where id= ".intval($v['deal_id']);
						}
						else
						{
							$sql_1 = "update ".DB_PREFIX."deal set buy_status = 1 where id= ".intval($v['deal_id']);
						}
						$GLOBALS['db']->query($sql);
						$GLOBALS['db']->query($sql_1);
					}
			...
			$refunded_total_score = $GLOBALS['db']->getOne("select sum(coupon_score) from ".DB_PREFIX."deal_coupon where order_id = ".$order_id." and refund_status = 2");
			$refunded_total_money = $GLOBALS['db']->getOne("select sum(coupon_money) from ".DB_PREFIX."deal_coupon where order_id = ".$order_id." and refund_status = 2");
				
			$refund_money = $order_info['pay_amount'] - $order_info['payment_fee'];  //订单的退款总额
			$return_user = $refund_money - $order_info['refund_amount'];  //退给用户的钱			
			$return_score = (0-$order_info['return_total_score'])+$refunded_total_score;
		...
		if(!$return_user)
		{
			if($after_sale>0)$log_info = "订单退款已处理  ".$admin_memo;
			else
			$log_info = $admin_memo;
		}
		else
		{	
			if($order_info['pay_status'] == 2)
			{
				$return_user-=$order_info['return_total_money'];
			}
			else
			{
				$return_score = 0;
			...
			//$return_user-=$order_info['return_total_money'];
			$data = array("money"=>$return_user,"score"=>$return_score);
			require_once APP_ROOT_PATH."system/libs/user.php";
			modify_account($data,$order_info['user_id'],"来自".$order_info['order_sn']."的退款 ".$admin_memo);
			$log_info = "订单退款 ".format_price($return_user)."  ".$admin_memo;			
			//处理返利
			if($order_info['pay_status'] == 2)
			{
				$res = return_referrals($order_id);
				if($res)
				{
					save_log("ID:".$order_id.l("REFERRALS_PAY_SUCCESS"),1);
				}
				else
				{
					save_log("ID:".$order_id.l("REFERRALS_PAY_FAILED"),0);
				}
			}			
			//更新团购劵的状态
			$GLOBALS['db']->query("update ".DB_PREFIX."deal_coupon set is_valid=2,refund_status = 2 where order_id=".$order_id." and confirm_time = 0");
			M("DealOrder")->where("id=".$order_id)->setField("refund_amount",$refund_money);
		}
	...
	public function referer()
	{
		if(isset($_REQUEST['referer'])&&trim($_REQUEST['referer'])!='')
		{
			$where = "referer = '".trim($_REQUEST['referer'])."' and is_delete = 0";
			$map['referer'] = array("eq",trim($_REQUEST['referer']));
			$map['is_delete'] = 0;
		}
		else
		{
