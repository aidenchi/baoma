...
class publishModule extends BizBaseModule
{
	...
	public function index()
	{
		...
		*/
		...
		{
			//$wap_url = url("wap/index","merchantitem#index",array("id"=>$item['id']));
			//$wap_url = get_domain().str_replace(APP_ROOT, APP_ROOT."/wap", $wap_url);
			
			$wap_url = get_domain().APP_ROOT."/wap/index.php?clt=merchantitem&id=".intval($item['id']);// str_replace(APP_ROOT, APP_ROOT."/wap", $wap_url);
			//$list[$k]['name'] = $wap_url;
			$list[$k]['wap_qrcode'] = gen_qrcode($wap_url);
			$list[$k]['wap_url'] = $wap_url;
		}
