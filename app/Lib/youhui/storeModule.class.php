...
// 			$deal_type_name = $GLOBALS['db']->getOne("select name from ".DB_PREFIX."deal_cate_type where id = ".$deal_type_id);
	   ...
	   if($keyword){
			...
			//有筛选
			$condition .=" and (tags_match_row like '%".$keyword."%' or  locate_match_row like '%".$keyword."%' or name like '%".$keyword."%') ";	
	   }
	...
	public static function get_reivew_form($arr){
	...
	public static function load_store_navs()
	{
...
//		
		...
		switch($type)
		{
			case "t":
				...
				$t_sql = "select id,name,img,uname from ".DB_PREFIX."deal as d 
					left join ".DB_PREFIX."deal_location_link as l on l.deal_id = d.id 
					where d.is_delete = 0 and d.is_effect = 1 and d.is_shop = 0 and d.time_status in (0,1) and l.location_id = ".$location_id." order by d.sort desc,d.id desc limit ".$limit;
			...
			case "d":
				...
				$d_sql = "select id,name,img,uname from ".DB_PREFIX."deal as d 
					left join ".DB_PREFIX."deal_location_link as l on l.deal_id = d.id 
					where d.is_delete = 0 and d.is_effect = 1 and d.is_shop = 2 and d.time_status in (0,1) and l.location_id = ".$location_id." order by d.sort desc,d.id desc limit ".$limit;
			...
			case "s":
				...
				$d_sql = "select id,name,img,uname from ".DB_PREFIX."deal as d 
					left join ".DB_PREFIX."deal_location_link as l on l.deal_id = d.id 
					where d.is_delete = 0 and d.is_effect = 1 and d.is_shop = 1 and d.time_status in (0,1) and l.location_id = ".$location_id." order by d.sort desc,d.id desc limit ".$limit;
				$result_list = $GLOBALS['db']->getAll($d_sql);
				$count = $GLOBALS['db']->getOne($d_sql_count);
				foreach($result_list as $k=>$v)
				{
			...
			case "y":				
				...
				$y_sql = "select id,name,icon from ".DB_PREFIX."youhui as y 
					left join ".DB_PREFIX."youhui_location_link as l on l.youhui_id  = y.id 
					where y.is_effect = 1 and l.location_id = ".$location_id.$time_condition." order by y.sort desc,y.id desc limit ".$limit;
			...
			case "e":
				...
				$e_sql = "select id,name,icon from ".DB_PREFIX."event as e 
					left join ".DB_PREFIX."event_location_link as l on l.event_id  = e.id 
					where e.is_effect = 1 and l.location_id = ".$location_id." order by e.sort desc,e.id desc limit ".$limit;
	...
	public static function load_recent_sign($param)
	{
		$id = intval($param['id']);
		$time = get_gmtime();
