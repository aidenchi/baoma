...
class fdetailModule extends YouhuiBaseModule
{
	...
	public function send_sms()
	{
		...
		else
		{
			...
			{	
				...
				if(!$youhui_info)
				{
					$result['info'] = $GLOBALS['lang']['YOUHUI_NO_EXIST'];
				 	ajax_return($result);
				}
				if( $youhui_info['total_num']>0 && (($youhui_info['print_count']+$youhui_info['sms_count']) >= $youhui_info['total_num']) )
				{
					$result['info'] = $GLOBALS['lang']['YOUHUI_LIMIT_OVER'];
				 	ajax_return($result);
				}
				
				if($youhui_info['end_time']>0&&$youhui_info['end_time']<get_gmtime())
				{
					$result['info'] = $GLOBALS['lang']['YOUHUI_END_TIME_OVER'];
				 	ajax_return($result);
				}
				
				if($youhui_info['is_sms']==0)
				{
					$result['info'] = $GLOBALS['lang']['YOUHUI_NO_SUPPORT_SMS'];
				 	ajax_return($result);
				}
	...
	public function fprint()
	{
		...
		if($youhui_info['total_num'] <= $youhui_info['print_count'])
		{
			showErr('剩余数量不足');
		}
		$GLOBALS['tmpl']->assign("youhui_info",$youhui_info);		
