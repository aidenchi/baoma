...
class uc_orderModule extends ShopBaseModule
{
	...
	public function view()
	{
		...
		else
		{
			...
			$order_info['deal_list'] = $GLOBALS['db']->getAll("select doi.*,d.is_shop,d.is_refund from ".DB_PREFIX."deal_order_item as doi left join ".DB_PREFIX."deal as d on doi.deal_id = d.id where doi.order_id = ".$order_info['id']);
			$order_info['payment'] = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."payment where id = ".$order_info['payment_id']);
		}
		
		$order_info['is_refund']=1;
		foreach($order_info['deal_list'] as $k=>$v){//多个商品和成个订单的时候，如果其中有个商品不支持退款,那么这个订单就不支持退款
			$order_info['is_refund'] *=$v['is_refund'];
		}
		...
		$message_group = array($GLOBALS['lang']['ORDER_TYPE_1'],$GLOBALS['lang']['ORDER_TYPE_2']);
