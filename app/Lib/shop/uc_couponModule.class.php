...
class uc_couponModule extends ShopBaseModule
{
	public function index()
	{
		...
		$GLOBALS['tmpl']->assign("now",get_gmtime());
	...
	public function cancel_refund()
	{
		...
		if($coupon_data['any_refund']==1|| ($coupon_data['expire_refund']==1&&$coupon_data['end_time']>0&&get_gmtime()>$coupon_data['end_time']))
		{
				$GLOBALS['db']->query("update ".DB_PREFIX."deal_coupon set refund_status = 0 where id = ".$coupon_data['id']);
				$coupon_res = $GLOBALS['db']->getAll("select refund_status from ".DB_PREFIX."deal_coupon where refund_status=1 and order_id=".intval($coupon_data['order_id']));
			
				if(!$coupon_res)
				$GLOBALS['db']->query("update ".DB_PREFIX."deal_order set refund_status = 0 where id = ".intval($coupon_data['order_id']));
				
				showSuccess("已成功撤销退款申请");
		}
		else
		{
			showErr("操作失败");
	...
	public function view()
	{
		...
		$coupon_data = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_coupon where id = ".$coupon_id." and is_valid in(1,2) and is_delete = 0 and user_id = ".intval($GLOBALS['user_info']['id']));
		if($coupon_data)
		{
