...
class cartModule extends ShopBaseModule
{
	...
	public function check()
	{
		...
		$cart_deal_buytype = 0;
		//开始验证关于属性的库存
		$deal_attr_ids = $GLOBALS['db']->getAll("select deal_id,attr,name,attr_str,buy_type from ".DB_PREFIX."deal_cart where session_id = '".es_session::id()."' and user_id = ".$GLOBALS['user_info']['id']);
	...
	public function done()
	{
		...
		//验证购物车
			...
			foreach($deal_ids as $row)
			{
				...
				$checker = check_deal_number($row['deal_id'],0,'done');
				if($checker['status']==0)
				{
					showErr($checker['info']." ".$GLOBALS['lang']['DEAL_ERROR_'.$checker['data']],$ajax,url("shop","cart#index"));
				}
	...
	public function order_done()
	{
		...
		{
			showErr($GLOBALS['lang']['REFUNDING_CANNOT_PAY']);
		}
		if($order['refund_status'] == 2)
		{
			showErr($GLOBALS['lang']['REFUNDED_CANNOT_PAY']);
		}
		$region4_id = intval($_REQUEST['region_lv4']);
		$region3_id = intval($_REQUEST['region_lv3']);
		$region2_id = intval($_REQUEST['region_lv2']);
		...
		if($order['pay_status'] == 0)
		{
			...
			foreach($deal_s as $row)
			{
				...
				$checker = check_deal_number($row['deal_id'],0,'no_pay');
				if($checker['status']==0)
				{
					showErr($checker['info']." ".$GLOBALS['lang']['DEAL_ERROR_'.$checker['data']],$ajax,url("shop","cart#index"));
