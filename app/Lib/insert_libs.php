...
{
	$rel_table = trim(addslashes($_REQUEST['act']));
...
function insert_load_youhui_comment()
{
	...
	require_once APP_ROOT_PATH.'app/Lib/page.php';
		...
		if($is_buy==1)
		{
			$m_type = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."message_type where type_name='after_sale'");
		}
		else
		{
			$m_type = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."message_type where type_name='before_sale'");
		}
		if($m_type["is_effect"]==1)
		{
			$result = get_message_list_shop($limit," rel_table='deal' and user_id =".$GLOBALS['user_info']["id"]." and  rel_id = ".$goods_id." and is_buy = ".$is_buy);
		}
		else
		{
