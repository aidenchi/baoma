 ...
 * 获取当前团购城市
 */
function get_current_deal_city($use_cookie = true)
{		
	if($use_cookie && es_cookie::is_set("deal_city"))
	{	
...
function init_nav_list($nav_list)
{
	...
	{			
		if($v['url']=='')
		{
				...
				if($v['u_module'] == 'article'){
					$v['u_param']="id=".$v['u_param'];
				}
				if($v['u_action']=='')
				$v["u_action"] = "index";
...
function get_order_item_link($order_item_id)
{
	...
	{
		$order = $GLOBALS['db']->getRow("select deal_id,name from ".DB_PREFIX."deal_order_item where id = ".$order_item_id);
		if($order)
		{
			return "<a href='".url("shop","goods",array("id"=>$order['deal_id']))."' target='_blank'>".$order['name']."</a>";
		}
		else
		{
