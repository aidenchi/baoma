<?php
	...
	function get_user_coupon($limit,$user_id,$status=0)
	{
		...
		$list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal_coupon where user_id = ".$user_id." and is_delete = 0 and is_valid in(1,2)".$ext_condition." order by order_id desc limit ".$limit);
		$count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal_coupon where user_id = ".$user_id." and is_delete = 0 and is_valid in(1,2)".$ext_condition);
		foreach($list as $k=>$v)
		{
			...
			$list[$k]['qrcode'] = gen_qrcode($v['password']);
		}
		return array("list"=>$list,'count'=>$count);		
	}
...
function get_tuan_collect_list($limit,$user_id)
	{
		$user_id = intval($user_id);
		$sql = "select d.*,c.create_time as add_time ,c.id as cid from ".DB_PREFIX."tuan_collect as c left join ".DB_PREFIX."deal as d on d.id = c.sup_id where c.user_id = ".$user_id." order by c.create_time desc limit ".$limit;
		$sql_count = "select count(*) from ".DB_PREFIX."tuan_collect where user_id = ".$user_id;
		$list = $GLOBALS['db']->getAll($sql);
		$count = $GLOBALS['db']->getOne($sql_count);
		return array("list"=>$list,'count'=>$count);
	}
