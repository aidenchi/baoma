...
//				alert(ajaxobj.responseText);
	...
	$("#Ajax_Login_Box .ajax_login_tabs .tab").live("click",function(){
		$("#Ajax_Login_Box .ajax_login_tabs .tab").removeClass("cur");
		$(this).addClass("cur");
		var rel = parseInt($(this).attr("rel"));
		if(rel==1){
			$("#Ajax_Login_Box #J_normal_login").addClass("hide");
			$("#Ajax_Login_Box #J_mobile_login").removeClass("hide");
		}
		else{
			$("#Ajax_Login_Box #J_mobile_login").addClass("hide");
			$("#Ajax_Login_Box #J_normal_login").removeClass("hide");
		}
	});
	
	$("#Ajax_Login_Box #settings-mobile").live("keyup",function(){
		var obj = $(this);
		if($.checkMobilePhone(obj.val()) && obj.val()!="" && $.maxLength($("#settings-mobile").val(),11,true))
		{
			$("#Ajax_Login_Box #get_regsms_code").removeClass("btn_disable");
		}
		else{
			$("#Ajax_Login_Box #get_regsms_code").addClass("btn_disable");
		}
	});
	
	$("#Ajax_Login_Box #get_regsms_code").live("click",function(){
			if(is_lock_send_vy || $(this).hasClass(".btn_disable")){
				return false;
			}
			is_lock_send_vy = true;
			
			var ajaxurl = APP_ROOT+"/sjmapi/index.php?act=register_verify_phone";
...
function left_time_to_send_regvy(){
	...
	if(left_rg_time > 0){
		...
		$("#Ajax_Login_Box #get_regsms_code").val(left_rg_time+"秒后重新获取验证码");
		$("#Ajax_Login_Box #get_regsms_code").addClass("btn_disable");
		left_rg_time -- ;
	}
	else{
		is_lock_send_vy = false;
		$("#Ajax_Login_Box #get_regsms_code").removeClass("btn_disable");
		$("#Ajax_Login_Box #get_regsms_code").val("重新获取验证码");
		left_rg_time = 0;
	}
}
...
function submit_buy()
{
	...
	var query = new Object();
	//判断是否团购
	var hd_is_coupon = $("input[name='hd_is_coupon']").val();
	if(hd_is_coupon == 1)
	{
		//填写手机号
		var telphone = $("input[name='coupon_mobile']").val();
...
function collect_supplier(sup_id)
{
	var ajaxurl = APP_ROOT+"/tuan.php?ctl=ajax&act=collect&id="+sup_id;
...
function init_auto_tag()
{
	var auto_tags = $("img[auto_tag='1']");
	for(var i=0;i<auto_tags.length;i++)
	{
		var auto_order = $(auto_tags[i]).attr("auto_order");
		var multi_attr = $(auto_tags[i]).attr("multi_attr");
		var is_today = $(auto_tags[i]).attr("is_today");
		var toppx = $(auto_tags[i]).position().top-4;
		var leftpx = $(auto_tags[i]).position().left;
		if(auto_order==1)
		{
			var auto_order_dom = $("<div class='tag_auto_order'></div>");
			$(auto_order_dom).css("top",toppx);
			$(auto_order_dom).css("left",leftpx);
			$(auto_tags[i]).parent().append(auto_order_dom);
			leftpx+=40;
		}
		
		if(multi_attr==1)
		{
			var multi_attr_dom = $("<div class='tag_multi_attr'></div>");
			$(multi_attr_dom).css("top",toppx);
			$(multi_attr_dom).css("left",leftpx);
			$(auto_tags[i]).parent().append(multi_attr_dom);
			leftpx+=40;
		}
		
		if(is_today==1)
		{
			var is_today_dom = $("<div class='tag_is_today'></div>");
			$(is_today_dom).css("top",toppx);
			$(is_today_dom).css("left",leftpx);
			$(auto_tags[i]).parent().append(is_today_dom);
			leftpx+=40;
		}
	...
	//修正deal页
	var auto_tags = $("img[auto_tag_deal='1']");
	for(var i=0;i<auto_tags.length;i++)
	{
		var auto_order = $(auto_tags[i]).attr("auto_order");
		var multi_attr = $(auto_tags[i]).attr("multi_attr");
		var is_today = $(auto_tags[i]).attr("is_today");
		var toppx = $(auto_tags[i]).offset().top-4;
		var leftpx = $(auto_tags[i]).offset().left;
		if(auto_order==1)
		{
			var auto_order_dom = $("<div class='tag_auto_order'></div>");
			$(auto_order_dom).css("top",toppx);
			$(auto_order_dom).css("left",leftpx);
			$("body").append(auto_order_dom);
			leftpx+=40;
		}
