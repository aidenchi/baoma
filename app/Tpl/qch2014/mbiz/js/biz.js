$(document).ready(function(){
	...
	$("form[name='supplier_login']").bind("submit",function(){
		...
		{
			show_msg_only_confirm("提示","请输入商家帐号");
			//$.showErr("请输入商家帐号");
		...
		if($.trim(account_password)=='')
		{
			...
			show_msg_only_confirm("提示","请输入商家密码");
			//$.showErr("请输入商家密码");
		...
		var ajaxurl = APP_ROOT+"/mbiz.php?ctl=login&act=dologin";
		$.ajax({ 
			url: ajaxurl,
			type: "POST",
			dataType: "json",
			...
			success: function(ajaxobj){
				...
				{
					show_msg_only_confirm("提示",ajaxobj.msg);
					//$.showErr(ajaxobj.msg);
				}
			},
...
//				alert(ajaxobj.responseText);
	...
	$("#add_coupon").click(function(){
		...
		var ajaxurl = APP_ROOT+"/mbiz.php?ctl=verify&act=add_coupon&coupon_pw="+coupon_pw;
		$.ajax({ 
			url: ajaxurl,
...
function use_coupon()
{
	...
	var ajaxurl = APP_ROOT+"/mbiz.php?ctl=verify&act=check_coupon&coupon_sn="+coupon_sn+"&coupon_pwd="+coupon_pwd;
	$.ajax({ 
		url: ajaxurl,
		dataType: "json",
		success: function(obj){
			if(obj.status==2)//未登录
			{
				show_msg_only_confirm("提示","请先登录");
				//$.showErr("请先登录");
			}
			if(obj.status == 0)
			{
				show_msg_only_confirm("提示",obj.msg);
				//确认失败
				//$.showErr(obj.msg);
			}
			if(obj.status == 1)
			{
				...
				{
					var ajaxurl = APP_ROOT+"/mbiz.php?ctl=verify&act=use_coupon&coupon_sn="+coupon_sn+"&coupon_pwd="+coupon_pwd;
					$.ajax({ 
						url: ajaxurl,
						dataType: "json",
						success: function(result){
							if(result.status)
							{
								show_msg_only_confirm("提示",result.msg);
								//$.showSuccess(result.msg);
							}
							else
								show_msg_only_confirm("提示",result.info);
								//$.showErr(result.info);
						},
						error:function(ajaxobj)
						{
...
function use_youhui()
{
	...
	var ajaxurl = APP_ROOT+"/mbiz.php?ctl=verify&act=check_youhui&youhui_sn="+youhui_sn;
	$.ajax({ 
		url: ajaxurl,
		dataType: "json",
		success: function(obj){
			if(obj.status)
			{
				if(confirm(obj.info))
				{
					var ajaxurl = APP_ROOT+"/mbiz.php?ctl=verify&act=use_youhui&youhui_sn="+obj.data.youhui_sn+"&total_fee="+total_fee;
					$.ajax({ 
						url: ajaxurl,
						dataType: "json",
						success: function(result){
							if(result.status)
							{
								show_msg_only_confirm("提示",result.data.youhui_data.name+"[序列号"+result.data.youhui_sn+"]消费使用成功");
...
//							alert(ajaxobj.responseText);
			...
			show_msg_only_confirm("提示",obj.info);
			//$.showErr(obj.info);
		},
		error:function(ajaxobj)
		{
...
function load_deal_cate_type()
{
	...
	var ajaxurl = APP_ROOT+"/mbiz.php?ctl=tuan&act=load_deal_cate_type&cate_id="+cate_id+"&deal_id="+deal_id+"&youhui_id="+youhui_id;
	$.ajax({ 
		url: ajaxurl,
		dataType: "json",
		success: function(result){
...
{
	var ajaxurl = APP_ROOT+"/mbiz.php?ctl=event&act=add_submit_item";
	$.ajax({ 
		url: ajaxurl,
		success: function(obj){
...
function use_coupon_batch(){
	...
	var ajaxurl = APP_ROOT+"/mbiz.php?ctl=verify&act=check_coupon_batch";
	$.ajax({ 
		url: ajaxurl,
		type: "POST",
		data:query,
